			...
			for (int j=0; j<numMaquinas; j++){
				...
				while (seq_pop[i][j][cont]!=-2) {	
					...
					if (seq_pop[i][j][cont+1]!=-2) {
						aux_makespan = aux_makespan + (int)matrizTarefaMaquina[seq_pop[i][j][cont]][j]+(int)matrizSetup[j][seq_pop[i][j][cont]][seq_pop[i][j][cont+1]];
					}else {
						aux_makespan = aux_makespan + (int)matrizTarefaMaquina[seq_pop[i][j][cont]][j];
					}					
					cont++;
				}
